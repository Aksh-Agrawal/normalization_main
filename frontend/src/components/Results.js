...
const Results = ({ data }) => {
  ...
  // Prepare chart data
  const platformData = Object.entries(platformWeights || {}).map(([platform, weight]) => ({
    platform,
        ...
        {rankings && rankings.length > 0 && (
          <div>
            <div className="grid">
              ...
              <div className="metric">
                <div className="metric-value">
                  {rankings[0][3].toFixed(1)}
                </div>
                <div className="metric-label">Total Rating</div>
              </div>
              <div className="metric">
                <div className="metric-value">
                  {rankings.length > 1 ? `#${rankings.findIndex(r => r[0] === userId) + 1}` : '#1'}
                </div>
    ...
    <div className="card">
      ...
      {platformData.length > 0 ? (
        <div>
          ...
          <div>
            <h3 style={{ marginBottom: '16px', color: '#333' }}>Platform Details</h3>
            {platformData.map((platform, index) => (
              <div key={platform.platform} className="platform-card">
                <div className="platform-name">{platform.platform}</div>
                <div className="platform-rating">Weight: {platform.weight}%</div>
                <div style={{ fontSize: '0.9rem', color: '#666', marginTop: '4px' }}>
                  This platform contributes {platform.weight}% to your unified rating
                </div>
              </div>
            ))}
          </div>
      ...
      {courseBonus > 0 ? (
        <div>
          <div className="success">
            🎉 Great job! You earned {courseBonus.toFixed(1)} bonus points from your Coursera courses.
          </div>
          
          <div style={{ marginTop: '20px' }}>
            <h3 style={{ color: '#333', marginBottom: '16px' }}>Bonus Breakdown</h3>
            <div className="metric">
            ...
            <div style={{ marginTop: '20px', padding: '16px', background: '#f8f9fa', borderRadius: '8px' }}>
              <h4 style={{ marginBottom: '12px', color: '#333' }}>How Course Bonus is Calculated:</h4>
              <ul style={{ listStyle: 'none', padding: 0, color: '#666' }}>
                <li>🏛️ <strong>Institution Reputation:</strong> Top universities and tech companies get higher scores</li>
                <li>⏱️ <strong>Course Duration:</strong> Longer, more intensive courses earn more points</li>
                <li>🔥 <strong>Field Relevance:</strong> AI, Machine Learning, and Data Science courses are highly valued</li>
                <li>💡 <strong>Skills Market Value:</strong> In-demand skills like Python, React, and Cloud Computing boost your score</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div>
          <div className="error">
            No course bonus detected. Add your Coursera profile URL to get bonus points!
          </div>
          
          <div style={{ marginTop: '20px', padding: '16px', background: '#f8f9fa', borderRadius: '8px' }}>
            <h4 style={{ marginBottom: '12px', color: '#333' }}>💡 Boost Your Rating with Courses:</h4>
            <p style={{ color: '#666', marginBottom: '12px' }}>
              Complete courses from top institutions to earn bonus points:
            </p>
            <ul style={{ listStyle: 'none', padding: 0, color: '#666' }}>
              <li>🎯 Focus on AI, Machine Learning, and Data Science</li>
              <li>🏛️ Choose courses from Stanford, MIT, Google, or other top institutions</li>
              <li>⏱️ Longer specializations and certificate programs earn more points</li>
              <li>💪 Build in-demand skills like Python, Cloud Computing, and DevOps</li>
            </ul>
          </div>
        </div>
      )}
      ...
      <div className="card" style={{ marginTop: '30px' }}>
        ...
        <div style={{ color: '#666', lineHeight: '1.6' }}>
          ...
          <ul style={{ listStyle: 'none', padding: 0 }}>
            ...
            <li>🎯 Focus on platforms with higher weights in the calculation</li>
