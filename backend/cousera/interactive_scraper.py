...
def main():
    try:
        ...
        # Option to scrape another profile
        while True:
            another = input("\nDo you want to scrape another profile? (y/n): ").strip().lower()
            if another in ['y', 'yes']:
                profile_url = input("\nEnter Coursera profile URL: ").strip()
                
                # Validate URL
                while not validate_coursera_url(profile_url):
                    print(f"\nInvalid Coursera profile URL: {profile_url}")
                    profile_url = input("Please enter a valid Coursera profile URL: ").strip()
                ...
                save = input("\nDo you want to save these results to a file? (y/n): ").strip().lower()
                if save in ['y', 'yes']:
                    filename = input("Enter filename (or press Enter for default with timestamp): ").strip()
                    if not filename:
                        timestamp = int(time.time())
                        filename = f"coursera_profile_{timestamp}.json"
